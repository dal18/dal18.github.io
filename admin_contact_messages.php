...
<head>
    ...
    <title>Contact Messages - LFHS</title>
...
<body class="bg-gradient-to-br from-gray-50 to-gray-100 min-h-screen">
            ...
            <div class="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-4 gap-6">
                <div class="hover-lift rounded-2xl p-6 text-white shadow-xl gradient-blue">
                    <div class="flex items-center justify-between">
                        <div><p class="text-sm font-medium opacity-90 mb-1">Total Messages</p><p class="text-4xl font-bold" id="totalMessages">0</p></div>
                        <div class="bg-white/20 p-4 rounded-xl"><i class="fas fa-envelope text-3xl"></i></div>
                    </div>
                </div>
    ...
    $(function() {
        ...
        function load(keyword, perpage, page, date_filter) {
            $.ajax({ type: 'post', url: 'php/read_contact_messages.php', data: { keyword, perpage, page, date_filter }, beforeSend: () => $("#contact_messages_page").html('<div class="flex items-center justify-center py-20"><div class="text-center"><i class="fas fa-spinner fa-spin text-5xl text-blue-500 mb-4"></i><p class="text-gray-600">Loading...</p></div></div>')
            }).done(function(data) { $("#contact_messages_page").html(data); var total = parseInt($('#cnt').text()) || 0; $({countNum: 0}).animate({countNum: total}, { duration: 1000, step: function() { $('#totalMessages').text(Math.floor(this.countNum)); }}); });
        }
