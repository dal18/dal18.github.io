...
<head>
    ...
    <title>School Stories - LFHS</title>
...
<body class="bg-gradient-to-br from-gray-50 to-gray-100 min-h-screen">
            ...
            <div class="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-4 gap-6">
                <div class="hover-lift rounded-2xl p-6 text-white shadow-xl gradient-blue">
                    <div class="flex items-center justify-between">
                        <div><p class="text-sm font-medium opacity-90 mb-1">Total Stories</p><p class="text-4xl font-bold" id="totalStories">0</p></div>
                        <div class="bg-white/20 p-4 rounded-xl"><i class="fas fa-book-open text-3xl"></i></div>
                    </div>
                </div>
            ...
            <div class="glass-card rounded-2xl p-6 shadow-lg">
                <div class="grid grid-cols-1 md:grid-cols-12 gap-4">
                    ...
                    <div class="md:col-span-2 flex items-end">
                        <button id="add_story" class="w-full px-6 py-3 bg-gradient-to-r from-green-500 to-emerald-600 text-white font-semibold rounded-xl hover:from-green-600 hover:to-emerald-700 transform hover:scale-105 transition-all shadow-lg"><i class="fas fa-plus mr-2"></i>Add Post</button>
                    </div>
                    <?php } ?>
                </div>
            </div>
    ...
    $(function() {
        ...
        function load(keyword, perpage, page, date_filter) {
            $.ajax({ type: 'post', url: 'php/read_stories.php', data: { keyword, perpage, page, date_filter }, beforeSend: () => $("#stories_page").html('<div class="flex items-center justify-center py-20"><div class="text-center"><i class="fas fa-spinner fa-spin text-5xl text-blue-500 mb-4"></i><p class="text-gray-600">Loading...</p></div></div>')
            }).done(function(data) { $("#stories_page").html(data); var total = parseInt($('#cnt').text()) || 0; $({countNum: 0}).animate({countNum: total}, { duration: 1000, step: function() { $('#totalStories').text(Math.floor(this.countNum)); }}); });
        }
        ...
        $(document).on('click', '#add_story', function() { $("#modalTitle").html('<i class="fas fa-plus mr-2"></i>Add Post'); $("#modalBody").load('php/add_story.php'); $("#customModal").removeClass('hidden').addClass('flex'); });
