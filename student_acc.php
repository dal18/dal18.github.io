...
<head>
    ...
    <title>Student Achievements - LFHS</title>
...
<body class="bg-gradient-to-br from-gray-50 to-gray-100 min-h-screen">
        ...
        <header class="sticky top-0 z-40 glass-card shadow-lg border-b border-gray-200">
            <div class="flex items-center justify-between px-6 py-4">
                <div>
                    ...
                    <p class="text-sm text-gray-500 mt-1">Manage student achievements and awards</p>
                </div>
                <div class="relative">
            ...
            <div class="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-4 gap-6">
                <div class="hover-lift rounded-2xl p-6 text-white shadow-xl gradient-blue">
                    <div class="flex items-center justify-between">
                        <div><p class="text-sm font-medium opacity-90 mb-1">Total Achievements</p><p class="text-4xl font-bold" id="totalAch">0</p></div>
                        <div class="bg-white/20 p-4 rounded-xl"><i class="fas fa-trophy text-3xl"></i></div>
                    </div>
                </div>
                <div class="hover-lift rounded-2xl p-6 text-white shadow-xl gradient-green">
                    <div class="flex items-center justify-between">
                        <div><p class="text-sm font-medium opacity-90 mb-1">This Year</p><p class="text-4xl font-bold" id="yearAch">0</p></div>
                        <div class="bg-white/20 p-4 rounded-xl"><i class="fas fa-calendar-check text-3xl"></i></div>
                    </div>
                </div>
                ...
                <div class="hover-lift rounded-2xl p-6 text-white shadow-xl gradient-orange">
                    <div class="flex items-center justify-between">
                        <div><p class="text-sm font-medium opacity-90 mb-1">Recent</p><p class="text-4xl font-bold" id="recentAch">0</p></div>
            ...
            <div class="glass-card rounded-2xl p-6 shadow-lg">
                <div class="grid grid-cols-1 md:grid-cols-12 gap-4">
                    <div class="md:col-span-5">
                        ...
                        <input type="text" id="keyword" placeholder="Search achievements..." class="w-full px-4 py-3 rounded-xl border-2 border-gray-200 focus:border-blue-500 focus:ring-4 focus:ring-blue-500/20 outline-none transition-all">
                    </div>
                    ...
                    <div class="md:col-span-2 flex items-end">
                        <button id="add_student_ac" class="w-full px-6 py-3 bg-gradient-to-r from-green-500 to-emerald-600 text-white font-semibold rounded-xl hover:from-green-600 hover:to-emerald-700 transform hover:scale-105 transition-all shadow-lg"><i class="fas fa-plus mr-2"></i>Add Achievement</button>
                    </div>
                    <?php } ?>
                </div>
            </div>
            <div class="glass-card rounded-2xl shadow-lg overflow-hidden">
                <div id="student_acc_page">
    ...
    $(function() {
        ...
        function load(keyword, perpage, page, date_filter) {
            $.ajax({ type: 'post', url: 'php/read_student_acc.php', data: { keyword, perpage, page, date_filter }, beforeSend: () => $("#student_acc_page").html('<div class="flex items-center justify-center py-20"><div class="text-center"><i class="fas fa-spinner fa-spin text-5xl text-blue-500 mb-4"></i><p class="text-gray-600">Loading...</p></div></div>')
            }).done(function(data) { $("#student_acc_page").html(data); var total = parseInt($('#cnt').text()) || 0; $({countNum: 0}).animate({countNum: total}, { duration: 1000, step: function() { $('#totalAch').text(Math.floor(this.countNum)); }}); });
        }
        ...
        $(document).on('click', '#add_student_ac', function() { $("#modalTitle").html('<i class="fas fa-plus mr-2"></i>Add Achievement'); $("#modalBody").load('php/add_student_achive.php'); $("#customModal").removeClass('hidden').addClass('flex'); });
